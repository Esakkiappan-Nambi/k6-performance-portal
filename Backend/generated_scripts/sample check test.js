...
export default function () {
    // Scheduled Test | Frequency: once | Start: 2026-07-01T16:59 | End: 2026-07-01T17:01
    const variables = {};
    

    
        // Scheduler: wait until start time
        const scheduleStart = new Date("2026-07-01T16:59:00").getTime();
        if (Date.now() < scheduleStart) {
            const waitMs = scheduleStart - Date.now();
            console.log("Waiting until scheduled start:", "2026-07-01T16:59:00");
            sleep(waitMs / 1000);
        }

    
        // Scheduler: stop after end time
        const scheduleEnd = new Date("2026-07-01T17:01:00").getTime();
    ...
    for (let i = 0; i < 1; i++) {
        ...
        // ── GET cart ──
        let res_0 = http.get(`https://demowebshop.tricentis.com/cart`, {
            headers: {},
